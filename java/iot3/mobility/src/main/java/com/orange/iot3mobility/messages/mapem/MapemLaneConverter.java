...
public final class MapemLaneConverter {
    ...
    static List<LaneGeometry> toLaneGeometries(List<GenericLane> laneSet, Position3D refPoint) {
        ...
        for (GenericLane lane : laneSet) {
            ...
            List<String> directionalUse = lane.laneAttributes().directionalUse() != null
                    ? lane.laneAttributes().directionalUse()
                    : Collections.emptyList();
            List<String> typeFlags = extractTypeFlags(lane.laneAttributes().laneType());
    ...
    /**
     * Return the active flag list from the oneOf {@link LaneType} structure.
     * Exactly one sub-field should be non-null; if none is, returns an empty list.
     */
    private static List<String> extractTypeFlags(LaneType lt) {
        if (lt == null) return Collections.emptyList();
        if (lt.vehicle()        != null) return lt.vehicle();
        if (lt.crosswalk()      != null) return lt.crosswalk();
        if (lt.bikeLane()       != null) return lt.bikeLane();
        if (lt.sidewalk()       != null) return lt.sidewalk();
        if (lt.median()         != null) return lt.median();
        if (lt.striping()       != null) return lt.striping();
        if (lt.trackedVehicle() != null) return lt.trackedVehicle();
        if (lt.parking()        != null) return lt.parking();
        return Collections.emptyList();
    }
